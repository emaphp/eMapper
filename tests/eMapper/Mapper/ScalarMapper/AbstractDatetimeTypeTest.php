...
abstract class AbstractDatetimeTypeTest extends AbstractMapperTest {
	...
	public function testDatetimeList() {
		$values = $this->mapper->type('DateTime[]')->query("SELECT birth_date FROM users ORDER BY user_id ASC");
	...
	public function testDatetimeColumnList() {
		$values = $this->mapper->type('DateTime[]', 'last_login')->query("SELECT * FROM users ORDER BY user_id ASC");
	...
	public function testDatetimeError1() {
		$result = $this->mapper->type('DateTime')->query("SELECT user_id FROM users WHERE user_id = 1");
	}
	...
	public function testDatetimeError2() {
		$result = $this->mapper->type('DateTime')->query("SELECT user_name FROM users WHERE user_id = 1");
	}
	...
	public function testDatetimeError3() {
		$result = $this->mapper->type('DateTime')->query("SELECT avatar FROM users WHERE user_id = 1");
	}
}
?>