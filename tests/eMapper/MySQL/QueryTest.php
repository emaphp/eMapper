...
class QueryTest extends AbstractQueryTest {
	...
	public function testRegex() {
		...
		$this->assertRegExpMatch("/SELECT _t.\* FROM @@products _t WHERE _t.product_code REGEXP BINARY #\{([\w]+)\}/", $query, $matches);
		$index = $matches[1];
	...
	public function testNotRegex() {
		...
		list($query, $args) =  $query->build($this->driver, []);
		$this->assertRegExpMatch("/SELECT _t.\* FROM @@products _t WHERE _t.product_code NOT REGEXP BINARY #\{([\w]+)\}/", $query, $matches);
		$index = $matches[1];
		$this->assertArrayHasKey($index, $args);
		$this->assertEquals('^(An?|The) +', $args[$index]);
	}
