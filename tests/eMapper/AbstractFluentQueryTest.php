...
abstract class AbstractFluentQueryTest extends MapperTest {
	public function testColumns() {
		...
		$sql = $query->from('users')->select('id', 'email')->build();
		$this->assertEquals("SELECT id,email FROM users", $sql);
		
		$query = $this->mapper->newQuery();
		$sql = $query->from('users', 'u')->select('u.id', 'u.email')->build();
		$this->assertEquals("SELECT u.id,u.email FROM users u", $sql);
		
		$query = $this->mapper->newQuery();
		$sql = $query->from('users', 'u')->select('id', 'email')->build();
		$this->assertEquals("SELECT id,email FROM users u", $sql);
		
		$query = $this->mapper->newQuery();
		$sql = $query->from('users')->select(Column::id(), Column::email())->build();
		$this->assertEquals("SELECT id,email FROM users", $sql);
		
		$sql = $query->from('users', 'u')->select(Column::id(), Column::email())->build();
		$this->assertEquals("SELECT u.id,u.email FROM users u", $sql);
		
		$query = $this->mapper->newQuery();
		$sql = $query->from('users')->select(Column::users__id(), Column::users__email())->build();
		$this->assertEquals("SELECT users.id,users.email FROM users", $sql);
		
		$query = $this->mapper->newQuery();
		$sql = $query->from('users', 'u')->select(Column::users__id(), Column::users__email())->build();
		$this->assertEquals("SELECT u.id,u.email FROM users u", $sql);
	}
	
	public function testOrderBy() {
		$query = $this->mapper->newQuery();
		$sql = $query->from('users')->orderBy('id')->build();
		$this->assertEquals("SELECT * FROM users ORDER BY id", $sql);
		
		$query = $this->mapper->newQuery();
		$sql = $query->from('users')->orderBy('users.id')->build();
		$this->assertEquals("SELECT * FROM users ORDER BY users.id", $sql);
		
		$query = $this->mapper->newQuery();
		$sql = $query->from('users', 'u')->orderBy('id')->build();
		$this->assertEquals("SELECT * FROM users u ORDER BY id", $sql);
		
		$query = $this->mapper->newQuery();
		$sql = $query->from('users')->orderBy(Column::id())->build();
		$this->assertEquals("SELECT * FROM users ORDER BY id", $sql);
		
		$query = $this->mapper->newQuery();
		$sql = $query->from('users', 'u')->orderBy(Column::id())->build();
		$this->assertEquals("SELECT * FROM users u ORDER BY u.id", $sql);
		
		$query = $this->mapper->newQuery();
		$sql = $query->from('users')->orderBy(Column::users__id())->build();
		$this->assertEquals("SELECT * FROM users ORDER BY users.id", $sql);
		
		$query = $this->mapper->newQuery();
		$sql = $query->from('users', 'u')->orderBy(Column::users__id())->build();
		$this->assertEquals("SELECT * FROM users u ORDER BY u.id", $sql);
		
		//order by + type
		$query = $this->mapper->newQuery();
		$sql = $query->from('users')->orderBy('id ASC')->build();
		$this->assertEquals("SELECT * FROM users ORDER BY id ASC", $sql);
		
		$query = $this->mapper->newQuery();
		$sql = $query->from('users')->orderBy('users.id DESC')->build();
		$this->assertEquals("SELECT * FROM users ORDER BY users.id DESC", $sql);
		
		$query = $this->mapper->newQuery();
		$sql = $query->from('users', 'u')->orderBy('id ASC')->build();
		$this->assertEquals("SELECT * FROM users u ORDER BY id ASC", $sql);
		
		$query = $this->mapper->newQuery();
		$sql = $query->from('users')->orderBy(Column::id('ASC'))->build();
		$this->assertEquals("SELECT * FROM users ORDER BY id ASC", $sql);
		
		$query = $this->mapper->newQuery();
		$sql = $query->from('users', 'u')->orderBy(Column::id('DESC'))->build();
		$this->assertEquals("SELECT * FROM users u ORDER BY u.id DESC", $sql);
		
		$query = $this->mapper->newQuery();
		$sql = $query->from('users')->orderBy(Column::users__id('ASC'))->build();
		$this->assertEquals("SELECT * FROM users ORDER BY users.id ASC", $sql);
		
		$query = $this->mapper->newQuery();
		$sql = $query->from('users', 'u')->orderBy(Column::users__id('DESC'))->build();
		$this->assertEquals("SELECT * FROM users u ORDER BY u.id DESC", $sql);
	}
	
	public function testLimitOffset() {
		$query = $this->mapper->newQuery();
		$sql = $query->from('users')->limit(10)->build();
		$this->assertEquals("SELECT * FROM users LIMIT 10", $sql);
		
		$query = $this->mapper->newQuery();
		$sql = $query->from('users')->offset(10)->build();
		$this->assertEquals("SELECT * FROM users OFFSET 10", $sql);
		
		$query = $this->mapper->newQuery();
		$sql = $query->from('users')->limit(10)->offset(5)->build();
	...
	public function testJoins() {
		...
		$query = $this->mapper->newQuery();
		$sql = $query->from('users', 'u')
		->innerJoin('profiles', 'p', Column::profiles__name()->eq(Column::users__lastname()))
		->build();
