 ...
 * @group callback
 */
class FilterCallbackTest extends SQLiteTest {
	...
	public function testSingleObject() {
		$value = self::$mapper->type('obj')->filter(function ($value) {
			return (intval($value->user_id) % 2) == 0;
	...
	public function testGroupedObjectList() {
		$values = self::$mapper->type('obj<category>')->filter(function ($value) {
			return (floatval($value->price) >= 150);
	...
	public function testIndexedGroupedObjectList() {
		$values = self::$mapper->type('obj<category>[product_id:i]')->filter(function ($value) {
			return (floatval($value->price) >= 150);
		})->query("SELECT * FROM products ORDER BY product_id ASC");
	...
	public function testIndexedArrayList() {
		$values = self::$mapper->type('arr[user_id:i]')->filter(function ($value) {
			return (intval($value['user_id']) % 2) == 0;
		})->query("SELECT * FROM users ORDER BY user_id ASC");
	...
	public function testIndexedGroupedArrayList() {
		$values = self::$mapper->type('arr<category>[product_id:i]')->filter(function ($value) {
			return (floatval($value['price']) >= 150);
		})->query("SELECT * FROM products ORDER BY product_id ASC");
