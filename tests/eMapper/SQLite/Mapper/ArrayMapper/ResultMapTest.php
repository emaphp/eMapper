...
/**
 * Test SQLiteMapper obtaining array values using result maps
 * @author emaphp
 * @group sqlite
 * @group mapper
 */
class ResultMapTest extends SQLiteTest {
	...
	public function testIndexedList() {
		$users = self::$mapper->type('array[name]')
		->result_map('Acme\Result\UserResultMap')
		->query("SELECT * FROM users ORDER BY user_id ASC");
	...
	public function testCustomIndexResultMapList() {
		$users = self::$mapper->type('array[user_id:string]')
		->result_map('Acme\Result\UserResultMap')
		->query("SELECT * FROM users ORDER BY user_id ASC");
	...
	public function testIndexOverrideList() {
		$products = self::$mapper->type('array[category]')
	...
	public function testGroupedIndexedList() {
		$products = self::$mapper->type('array<category>[code]')
		->result_map('Acme\Result\GenericProductResultMap')
		->query("SELECT * FROM products ORDER BY product_id ASC");
