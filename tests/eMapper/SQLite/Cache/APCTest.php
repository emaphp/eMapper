 ...
 * @group apc
 */
class APCTest extends SQLiteTest {
	/**
	 * SQLite mapper
	 * @var SQLiteMapper
	 */
	...
	public function testSetArrayList() {
		...
		$this->sqlite->cache('sqlite_set_arraylist', 60)->type('array[user_id:int]')->query("SELECT * FROM users ORDER BY user_id ASC");
	...
	public function testSetObjectList() {
		...
		$this->sqlite->cache('sqlite_set_objectlist', 60)->type('object[user_id:int]')->query("SELECT * FROM users ORDER BY user_id ASC");
