...
abstract class AbstractQueryTest extends \PHPUnit_Framework_TestCase {
	...
	public function testSelectAll() {
		...
		$this->assertEquals("SELECT _t.product_id,_t.product_code,_t.price,_t.category,_t.color FROM @@products _t", $query);
	...
	public function testSelectAllOrder() {
		...
		list($query, $args) = $query->build($this->driver, $config);
		$this->assertEquals("SELECT _t.product_id,_t.product_code,_t.price,_t.category,_t.color FROM @@products _t ORDER BY _t.product_id, _t.product_code DESC", $query);
	}
	...
	public function testSelectAllLimit() {
		...
		list($query, $args) = $query->build($this->driver, $config);
		$this->assertEquals("SELECT _t.product_id,_t.product_code,_t.price,_t.category,_t.color FROM @@products _t LIMIT 10", $query);
	}
	...
	public function testSelectAllLimits() {
		...
		list($query, $args) = $query->build($this->driver, $config);
		$this->assertEquals("SELECT _t.product_id,_t.product_code,_t.price,_t.category,_t.color FROM @@products _t LIMIT 5, 10", $query);
	}
	...
	public function testSelectAllOrderLimits() {
		...
		list($query, $args) = $query->build($this->driver, $config);
		$this->assertEquals("SELECT _t.product_id,_t.product_code,_t.price,_t.category,_t.color FROM @@products _t ORDER BY _t.product_id, _t.product_code DESC LIMIT 5, 10", $query);
	}
	...
	public function testSelectIContains() {
		...
		list($query, $args) = $query->build($this->driver, []);
		$this->assertRegExpMatch("/^SELECT _t\.product_id,_t\.product_code,_t\.price,_t\.category,_t\.color FROM @@products _t WHERE LOWER\(_t\.product_code\) LIKE LOWER\(#\{([\w]+)\}\)/", $query, $matches);
		$index = $matches[1];
		$this->assertArrayHasKey($index, $args);
		$this->assertEquals('%GFX%', $args[$index]);
	}
	...
	public function testSelectNotIContains() {
		...
		list($query, $args) = $query->build($this->driver, []);
		$this->assertRegExpMatch("/^SELECT _t\.product_id,_t\.product_code,_t\.price,_t\.category,_t\.color FROM @@products _t WHERE LOWER\(_t\.product_code\) NOT LIKE LOWER\(#\{([\w]+)\}\)/", $query, $matches);
		$index = $matches[1];
		$this->assertArrayHasKey($index, $args);
		$this->assertEquals('%GFX%', $args[$index]);
	}
	...
	public function testSelectIn() {
		...
		list($query, $args) = $query->build($this->driver, []);
		$this->assertRegExpMatch('/SELECT _t\.product_id,_t\.product_code,_t\.price,_t\.category,_t\.color FROM @@products _t WHERE _t\.product_id IN \(#\{([\w]+):int\}\)/', $query, $matches);
		$index = $matches[1];
		$this->assertArrayHasKey($index, $args);
	...
	public function testSelectNotIn() {
		...
		list($query, $args) = $query->build($this->driver, []);
		$this->assertRegExpMatch('/SELECT _t\.product_id,_t\.product_code,_t\.price,_t\.category,_t\.color FROM @@products _t WHERE _t\.product_id NOT IN \(#\{([\w]+):int\}\)/', $query, $matches);
		$index = $matches[1];
		$this->assertArrayHasKey($index, $args);
		$this->assertEquals([3, 4], $args[$index]);
	}
	...
	public function testSelectGreaterThan() {
		...
		list($query, $args) = $query->build($this->driver, []);
		$this->assertRegExpMatch('/SELECT _t\.product_id,_t\.product_code,_t\.price,_t\.category,_t\.color FROM @@products _t WHERE _t\.product_id > #\{([\w]+):int\}/', $query, $matches);
		$index = $matches[1];
		$this->assertArrayHasKey($index, $args);
	...
	public function testSelectRange() {
		...
		list($query, $args) = $query->build($this->driver, []);
		$this->assertRegExpMatch("/SELECT _t\.product_id,_t\.product_code,_t\.price,_t\.category,_t\.color FROM @@products _t WHERE _t\.product_id BETWEEN #\{([\w]+):int\} AND #\{([\w]+):int\}/", $query, $matches);
	...
	public function testSelectNotRange() {
		...
		list($query, $args) = $query->build($this->driver, []);
		$this->assertRegExpMatch("/SELECT _t\.product_id,_t\.product_code,_t\.price,_t\.category,_t\.color FROM @@products _t WHERE _t\.product_id NOT BETWEEN #\{([\w]+):int\} AND #\{([\w]+):int\}/", $query, $matches);
		$from_index = $matches[1];
		$to_index = $matches[2];
		$this->assertArrayHasKey($from_index, $args);
		$this->assertArrayHasKey($to_index, $args);
