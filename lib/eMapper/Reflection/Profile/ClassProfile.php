...
class ClassProfile {
	...
	public function __construct($classname) {
		...
		foreach ($this->propertiesAnnotations as $name => $attr) {
			if ($attr->has('eval')) {
				$this->dynamicAttributes[$name] = new MacroExpression($name, $attr);
			}
			elseif ($attr->has('stmt')) {
				$this->dynamicAttributes[$name] = new StatementCallback($name, $attr);
			}
			elseif ($attr->has('query')) {
				$this->dynamicAttributes[$name] = new QueryCallback($name, $attr);
			}
			elseif ($attr->has('procedure')) {
				$this->dynamicAttributes[$name] = new StoredProcedureCallback($name, $attr);
			}
			else {
